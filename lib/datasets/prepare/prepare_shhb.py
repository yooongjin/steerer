...
mode = 'train'
Root = '/home/cho092871/Desktop/Datasets/shanghaitech_part_B'
...
def resize_images(src_path, shift=0, resize_factor = 2):
    file_list = glob.glob(os.path.join(src_path,'img','*.jpg'))
    print(len(file_list))
...
def writer_jsons():
    ...
    for idx, img_name in enumerate(os.listdir(dst_imgs_path)):
        ...
        if os.path.exists(dst_json_name):
            ...
            size_map = cv.imread(imgPath.replace('img','size_map'),cv.IMREAD_GRAYSCALE)
            ...
            if img_id<='0400':
                ...
                ori_imgPath =  os.path.join(train_path,'img','IMG_'+ img_id + '.jpg')
            else:
                gt_path = os.path.join(test_path, 'ground_truth', 'GT_IMG_' + str(int(img_id)-400) + '.mat')
                ori_imgPath = os.path.join(test_path, 'img', 'IMG_' +str(int(img_id)-400)+ '.jpg')
